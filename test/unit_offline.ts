...
const soapLine = engine.appendSoap('plan', 'Discontinue ibuprofen');
ok('appendSoap stores one plan line', soapLine?.text === '• Discontinue ibuprofen');
ok('appendSoap dedupes the same line', engine.appendSoap('plan', 'Discontinue ibuprofen') === null);
const toolEngine = new ClinicalEngine();
const toolClient = new AssemblyAIVoiceAgentClient(undefined, { clinicalEngine: toolEngine, isMock: true });
await toolClient.executeToolCall({ call_id: 'soap_1', name: 'update_soap', arguments: { section: 'objective', text: 'Blood pressure 158/96 mmHg' } });
ok('update_soap tool writes the chart', toolEngine.soapNotes.objective.length === 1);
